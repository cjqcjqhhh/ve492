...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def chanceValue(self, gameState, depth, agnetIndex):
        ...
        for action in actions:
            if (agnetIndex != gameState.getNumAgents() - 1):
                vList.append(self.chanceValue(gameState.generateSuccessor(agnetIndex, action), 
                                              depth, agnetIndex + 1))
            else:
                vList.append(self.maxValue(gameState.generateSuccessor(agnetIndex, action),
                                            depth - 1, self.index))
...
def betterEvaluationFunction(currentGameState):
        ...
        if (scaredTime == 0):
            ...
            if (manhattanDistance(curPos, curGhostPos[i]) < curScaredTimes[i]):
                if (manhattanDistance(curPos, curGhostPos[i]) <= 1):
                    ...
                    score += 100 / manhattanDistance(curPos, curGhostPos[i]) * eatAdot

    # distance to food
    score -= 10 * eatAdot * len(curFoodList) # penalize for more food it lefts
    for food in curFoodList:
    ...
    score -= 5000 * eatAdot * len(curCapsules) # penalize for captures it lefts
    for curCapsule in curCapsules:
        if (manhattanDistance(curPos, curCapsule) <= 1):
            score += 1000 * eatAdot
        else:
            score += 100 / manhattanDistance(curPos, curCapsule) * eatAdot

    # --- specification handling --- #

    if (len(curFoodList) <= 3):
        score -= 5 * eatAdot * len(curFoodList)
        for food in curFoodList:
            if (manhattanDistance(curPos, food) <= 1):
                score += 10 * eatAdot
            else:
