...
def consistent(assignment, csp, var, value):
    ...
    for binaryConstraint in csp.binaryConstraints:
        if binaryConstraint.affects(var):
            ...
            if assignment.isAssigned(otherVar) and \
                not binaryConstraint.isSatisfied(value, assignment.assignedValues[otherVar]):
                return False
    return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    """
    nextVar = None
    domains = assignment.varDomains # domain for all variables

    # TODO: Question 2
    if assignment.isComplete():
        return nextVar
    
    unassignedVars = [] # a list of unassigned variables given the assignment
    for var in domains:
        if not assignment.isAssigned(var):
            unassignedVars.append(var)

    min = math.inf # min length of remaining values
    for var in unassignedVars:
        if len(domains[var]) < min:
            min = len(domains[var])
            nextVar = var
        elif len(domains[var]) == min:
            if countConstrainsInvolved(assignment, csp, var) < countConstrainsInvolved(assignment, csp, nextVar):
                nextVar = var

    return nextVar

def countConstrainsInvolved(assignment, csp, var):
    # helper function for minimumRemainingValuesHeuristic(assignment, csp)
    # to count the number of constraint involved given a variable
    num = 0
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for value in assignment.varDomains[var]:
        num = 0
        constraintAffect = [constraint for constraint in csp.binaryConstraints if constraint.affects(var)]
        for constraint in constraintAffect:
            ...
            for otherValue in assignment.varDomains[otherVar]:
                if not constraint.isSatisfied(value, otherValue):
                    num += 1
        countValues[value] = num
    
    countValues = dict(sorted(countValues.items(), key=lambda item: item[1])) # sort on values
    return list(countValues.keys())
...
def forwardChecking(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            otherVar = constraint.otherVariable(var) # variable connected to the assigned variable
            if value in assignment.varDomains[otherVar]:
                if len(assignment.varDomains[otherVar]) > 1:
                    inferences.add((otherVar, value)) # add to inference
                    assignment.varDomains[otherVar].remove(value) # remove inconsistant value
                else:
                    # if no value can be assigned to other variable
                    for inference in inferences:
                        assignment.varDomains[inference[0]].add(inference[1])
                    return None                
