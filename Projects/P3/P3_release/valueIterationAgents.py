...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        n = len(states)
        for i in range(self.iterations):
            thisState = states[i % n]
            if self.mdp.isTerminal(thisState):
                continue
            actions = self.mdp.getPossibleActions(thisState)
            self.values[thisState] = max([self.getQValue(thisState, action) for action in actions])
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    # Helper function for computing difference
    def computeDifference(self, state):
        actions = self.mdp.getPossibleActions(state)
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            ...
            for action in actions:
                for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
                    if nextState not in predecessor.keys():
        ...
        for state in self.mdp.getStates():
            if self.mdp.isTerminal(state):
                continue
            diff = self.computeDifference(state)
        ...
        for i in range(self.iterations):
            ...
            # perform pushing for each predecessors
            for p in predecessor[s]:
                diff = self.computeDifference(p)
                if diff > self.theta:
                    queue.update(p, -diff)
