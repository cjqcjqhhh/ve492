...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if noisyDistance is None:
            # indicate that the ghost must in jail
            if ghostPosition == jailPosition:
                return 1
            else:
                return 0
        else:
            # indicate that the ghost must not in jail
            if ghostPosition == jailPosition:
                return 0
            else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                temp[newPos] += newPosDist[newPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for legalPos in self.legalPositions:
            for _ in range(int(self.numParticles/len(self.legalPositions))):
                self.particles.append(legalPos) # append the particles in every legal postion uniformly
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # handle special case for all zero weight
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in beliefs.keys():
            for i in range(self.numGhosts):
                ...
                beliefs[particle] *= self.getObservationProb(observation[i], pacmanPosition, particle[i], jailPosition)
